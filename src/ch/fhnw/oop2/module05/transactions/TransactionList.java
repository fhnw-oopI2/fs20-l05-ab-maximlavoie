...
public final class TransactionList {
	...
	public List<Transaction> transactionsInYear(int year) {
		return allTransactions.stream().filter(n -> n.getYear() == year)
				.sorted(Comparator.comparing(t->t.getValue())).collect(Collectors.toList());
	}
	...
	public boolean traderInCity(String city) {
		...
		return allTransactions.stream().filter(t -> t.getTrader().getCity() == city).count() != 0;
	}
	...
	public void relocateTraders(String from, String to) {
		allTransactions.stream().filter(t -> t.getTrader().getCity() == from).forEach(t -> t.getTrader().setCity(to));
	}
	...
	public int highestValue() {
		return allTransactions.stream().max(Comparator.comparing(t -> t.getValue())).map(t -> t.getValue()).orElse(0);
